...
| **F-05a** | **Call `ReportingLineValidator::canAssign()` from EVERY write path that sets `reporting_manager_id`** | Q-B1 | 2, 4, 9 | **S** · employee create/edit · onboarding · bulk import · admin screens | the guarantee itself | F-05 | AT-F05a | API | ✅ **DONE via X-16** (2026-08-11). It could not be done first — **there were no write paths to add the call to.** X-16 built the path and the smoke suite asserts there is exactly ONE, that it is guarded, and that nothing else in `app/` writes the column |
| **F-05b** | **Manager assignment mechanism** — bulk and individual, for `reporting_manager_id` **and** `head_user_id` | Q-B1 | 2, 4, 9 | **M** | Slice 2's demo | F-05, F-05a | AT-F05b | SCREEN | ✅ **DONE via X-16** (2026-08-11). It was the PREREQUISITE for F-05a, not its dependant |
...
| ~~**F-10**~~ **F-10a** | **`s_user_jobrole_task.catalogue_task_id`** — keyed on **(jobrole, task)**, remainder HELD at NULL | — | **2** | **M** · 85,660 rows, 93.62% keyable | the chain's last name-join | — | AT-F10 | DB | 🔴 **OPEN** — ~~TIME-SENSITIVE~~ **NOT time-sensitive: the decay premise was disproved by measurement.** Nothing here decays; the 126 non-matching rows are one tenant that authored its own tasks and would not have matched on day one |
| **F-10b** | **3,127 duplicate `(jobrole, task)` pairs IN THE GLOBAL CATALOGUE** — what are they? | — | **2** | **S** · a read, not a migration | caps any key from these columns at ~93.6% | F-10a | — | DB | 🔴 **OPEN** — a data-quality question, deliberately NOT folded into F-10a |
