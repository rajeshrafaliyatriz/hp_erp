<?php
/** G-SEC-29: the last five, after. Self-mutation pre-check first, then tenants. */
...
foreach (['CustomModuleController@menuLevel2','HolidayController@index','discliplinaryManagementController@index',
          'jobroletaskcontroller@index','jobroletexonomycontroller@index'] as $a) {
    $uri = $uriOf[$a] ?? null; if (!$uri) { printf("  %-44s NO URI\n", $a); continue; }
    [$sa,$ba] = $call($uri,7); [$sb,$bb] = $call($uri,7);
    if ($sa === $sb && $ba !== $bb) { printf("  %-44s UNJUDGEABLE (varies from itself)\n", $a); continue; }
