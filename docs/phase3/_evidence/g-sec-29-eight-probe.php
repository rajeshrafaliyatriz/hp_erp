<?php
/**
 * G-SEC-29: the eight `session() ?? $request` controllers, before and after.
 * Same harness that found the defect. All eight pre-cleared STABLE by the
 * self-mutation check, so a differing response here IS a tenant difference.
 */
...
$targets = ['ApplyLeaveController@index','LeaveSummaryReportController@leaveSummaryReportShow',
            'LeaveTypeController@index','courseController@index','questionmasterController@indexChapter',
            'skillcontroller@index','sub_std_mapController@index','taskController@taskAnalysisReport'];
...
foreach ($targets as $a) {
    $uri = $uriOf[$a] ?? null; if (!$uri) { printf("  %-52s NO URI\n", $a); continue; }
