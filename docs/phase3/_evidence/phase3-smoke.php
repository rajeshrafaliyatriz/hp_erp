...
echo "\n";
echo "  - TIME. THE SUITE RUNS AT ONE INSTANT AND THE SYSTEM DOES NOT.\n";
echo "    Every check above is one request, one query, one moment. Four behaviours\n";
echo "    are claims about a SEQUENCE, and each has a check that passes on its\n";
echo "    one-instant behaviour:\n";
echo "        hysteresis            a gate that must hold for N sustained periods\n";
echo "        certification expiry  a credential that lapses on a date\n";
echo "        delivery idempotency  a projector re-run across DELIVERIES, not calls\n";
echo "        never-auto-lowers     proficiency that may rise but not fall\n";
echo "    IT IS NOT THAT THE TIME DIMENSION FAILS; IT IS NEVER ASKED.\n";
echo "\n";
echo "    THE THREE ABOVE ARE VISIBLY ABSENT - anyone can see 912 routes are not\n";
echo "    run. THIS ONE IS INVISIBLY ABSENT, which is why it was the last thing\n";
echo "    found and not the first. A STATED LIMIT, NOT A DEFECT: nothing here is\n";
echo "    known to be broken, and nothing here is known to be right.\n";
