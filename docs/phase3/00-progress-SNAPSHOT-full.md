...
1. `06-feature-audit/competency-library.md` — Competency Library (77 rows; writes
   the same `s_users_skills` through a different endpoint, inherits Gate A's D2)
2. `06-feature-audit/competency-framework-mapping.md` — Framework & Role Mapping
3. `06-feature-audit/competency-employee-profiles.md` — Employee Profiles

**C9 falls due after step 2** — report elements audited, turns taken, and a
projection for the remaining units.
...
| C | `06-feature-audit/*` write-ups | **In Progress** | **1 delivered** (Library & Taxonomy, accepted as the C8 template). C2 order: Competency → Organization → LMS → Task → Talent → rest |
| D | `08-connection-plan.md` | Not Started | |
| — | `09-implementation-log.md` | Not Started | Only after Gate D |
