...
|---|---|---|
| **Phase 14** | **The rest of the frontend list, rather than stopping at the worst of it.** Phase 13 fixed seven findings and left roughly twenty; asked to complete the module, this closed the ones a user meets. **Leave Reports offered fifteen reports against three endpoints** - nine had no backing at all, the preview showed the same leave-type summary whichever was selected, and Export wrote that summary under the *selected report's* filename, so "Holiday Calendar Report" downloaded leave-type totals and "Carry Forward" downloaded a balance CSV with no carry-forward column. Trimmed to the three that exist, and the preview now renders the dataset each one names. **Five columns on two attendance groupings could only ever print a dash**, because the data behind them is a day-count summary with no punch times. **Generate Payroll stayed enabled on a locked month** and the Lock/Reopen controls vanished on an empty one. **A page refresh permanently orphaned an issued salary certificate** - the document survived on the server with no control anywhere to retrieve it. Plus Print emitting a screenshot of the application rather than a report, and a failed payslip fetch reported to the employee as "no payslips". | `PHASE-14-FINISHING-THE-LIST.md` |
