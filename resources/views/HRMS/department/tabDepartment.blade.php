               ...
               <form action="{{route('institute_detail.store')}}" method="post" class="row mt-4" enctype="multipart/form-data">
                  ...
                  <div class="row cloneManageTask" id="cloneManageTask" style="padding:6px">
                     ...
                     <div class="col-md-4 form-group">
                        ...
                        <select name="arr[0][TASK_ALLOCATED_TO][]" id="selEmployees0" class="form-control selEmployees" data-val="0" required>
                           <option value="">Select Employee</option>
                           @if(!empty($data['departmentData']->employeesList))
                           @foreach($data['departmentData']->employeesList as $key=>$value)
                           <option value="{{$value->id ?? 0}}">{{$value->emp_name}}</option>
                           @endforeach
                           @endif
                        </select>
                     </div>
   ...
   $(document).ready(function(){
       ...
       $('#selEmployees'+dataVal).empty(); 
          $.ajax({
           ...
           type:"GET",
           success : function(response){
               $('#selEmployees'+dataVal).append(`<option value="">Select any one</option>`);
