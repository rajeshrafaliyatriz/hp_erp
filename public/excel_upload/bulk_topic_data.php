...
if (isset($_REQUEST['submit'])) {
    ...
    if (in_array($ext, $allowed)) {
        ...
        foreach ($dataArr as $key => $value) {
            if ($value['TopicName'] != "") {
                $checkQuery = "SELECT * FROM topic_master WHERE name = '" . $value['TopicName'] . "'
              AND chapter_id = '" . $_REQUEST['chapter'] . "' AND sub_institute_id = '" . $_REQUEST['sub_institute_id'] . "'";
                $result = mysqli_query($cn, $checkQuery) or die(mysqli_error($cn));

                if (mysqli_num_rows($result) == 0) {
                    $insertQuery = "INSERT INTO topic_master(syear,sub_institute_id,chapter_id,main_topic_id,name,description,
                topic_show_hide,topic_sort_order,created_at,created_by) values('" . $_REQUEST['syear'] . "','" . $_REQUEST['sub_institute_id'] . "','" . $_REQUEST['chapter'] . "',
                '0','" . $value['TopicName'] . "','" . $value['TopicDesc'] . "','" . $value['ShowHide'] . "','" . $value['SortOrder'] . "',now(),'" . $_REQUEST['user_id'] . "')";
                    //die;
                    ...
                    $problematic_reocords .= "Topic Name -  " . $value['TopicName'] . "<br><br>";
                }
            }
        }
    }
...
<div class="container">
    <form method="post" enctype="multipart/form-data">
        <div style="height:40px;background-color: #41b3f9;">
            <a style="float:right;" href="SampleTopicUpload.xlsx" download>Sample Excel File</a>
            <h4>Bulk Topic Upload</h4>
        </div>
        <br>
