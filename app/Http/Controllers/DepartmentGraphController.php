...
class DepartmentGraphController extends Controller
{
    ...
    public function show($deptId)
    {
        ...
        $result = $client->run(
            ...
            OPTIONAL MATCH (d)-[r1:BELONGS_TO_ORG]->(o:Organization)
            OPTIONAL MATCH (d)-[r2:HAS_BELONGS_TO_ORGPARENT]->(parent:Department)
            OPTIONAL MATCH (child:Department)-[r3:HAS_PARENT]->(d)
            OPTIONAL MATCH (j:JobRole)-[r4:IS_IN_DEPT]->(d)
            RETURN d, o, parent, child, j, r1, r2, r3, r4;',
            ['deptId' => is_numeric($deptId) ? (int)$deptId : $deptId]
        );
        ...
        foreach ($result as $record) {
            ...
            if ($record->get('o')) {
                $node = $this->formatNode($record->get('o'));
                $nodes[$node['id']] = $node;
            }
            ...
            if ($record->get('child')) {
                $node = $this->formatNode($record->get('child'));
                $nodes[$node['id']] = $node;
            }
